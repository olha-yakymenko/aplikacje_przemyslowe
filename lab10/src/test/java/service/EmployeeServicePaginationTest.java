...
class EmployeeServicePaginationTest {
    ...
    void testGetAllEmployees_withPagination() {
        ...
        // Then - zgrupowanie asercji dla wszystkich stron
        assertAll("Pagination verification for multiple pages",
    ...
    void testGetEmployeesByCompanyProjection_withPagination() {
        ...
        // Then - asercje dla wyników z projekcją
        assertAll("Company projection pagination verification",
                () -> assertThat(result.getTotalElements()).isGreaterThan(0),
                () -> {
    ...
    void testSearchEmployeesAdvanced_withPagination() {
        ...
        // Then - asercje dla zaawansowanego wyszukiwania
        assertAll("Advanced search pagination verification",
                () -> assertThat(result.getTotalElements()).isGreaterThan(0),
                () -> {
                    if (!result.getContent().isEmpty()) {
                        EmployeeListView first = result.getContent().get(0);
                        assertAll("First result verification",
    ...
    void testGetEmployeesByStatus_deprecated_withPagination() {
        ...
        // Then
        assertAll("Deprecated status method verification",
                () -> assertThat(result.getTotalElements()).isGreaterThan(0),
                () -> {
                    if (!result.getContent().isEmpty()) {
    ...
    void testGetEmployeesByCompany_deprecated_withPagination() {
        ...
        // Then
        assertAll("Deprecated company method verification",
                () -> assertThat(result.getTotalElements()).isGreaterThan(0),
                () -> {
                    if (!result.getContent().isEmpty()) {
                        assertThat(result.getContent().get(0).getCompany()).isEqualTo("TechCorp");
    ...
    void testSortingWithPagination() {
        ...
        for (int i = 0; i < employees.size() - 1; i++) {
            int finalI = i;
            assertAll(String.format("Sort verification for elements %d and %d", i, i + 1),
                    () -> assertThat(employees.get(finalI).getSalary())
                            .isGreaterThanOrEqualTo(employees.get(finalI + 1).getSalary())
            );
        }
    }
    ...
    void testSortingByNameWithPagination() {
        ...
        for (int i = 0; i < employees.size() - 1; i++) {
            int finalI = i;
            assertAll(String.format("Alphabetical sort verification for elements %d and %d", i, i + 1),
                    () -> {
                        String name1 = employees.get(finalI).getName();
                        String name2 = employees.get(finalI + 1).getName();
    ...
    void testGetEmployeeCountByCompany_withPagination() {
        ...
        // Then - zgrupowanie asercji dla obu firm
        assertAll("Company count verification with pagination",
    ...
    void testGetEmployeeCountByStatus_withPagination() {
        ...
        // Then
        assertAll("Status count verification",
    ...
    void testEmptyPage() {
        ...
        // Then - asercje dla pustej strony
        assertAll("Empty page verification",
    ...
    void testSingleEmployeePerPage() {
        ...
        // Then - asercje dla paginacji z 1 elementem na stronę
        assertAll("Single employee per page verification",
    ...
    void testNoSorting() {
        ...
        // Then - asercje dla braku sortowania
        assertAll("No sorting verification",
    ...
    void testCompanyStatistics_withPaginationVerification() {
        ...
        for (String company : companies) {
            ...
            // Grupowanie asercji dla każdej firmy
            assertAll(String.format("Statistics verification for company: %s", company),
    ...
    void testProjectionFields() {
        ...
        if (!result.getContent().isEmpty()) {
            ...
            // Grupowanie asercji dla pól projekcji
            assertAll("Projection fields verification",
    ...
    void testProjectionConsistency() {
        ...
        assertAll("Projection consistency verification",
